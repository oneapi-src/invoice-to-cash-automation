...
#from utils import trans  # pylint: disable=no-name-in-module
import trans
debug_idx = 0
debug = True
...
class MyDatasetPro(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index >= self.txtline_len:
            ...
            if index % 100 == 0:
                img.save('test_imgs/debug-{}-{}.jpg'.format(index, label.strip()))
        else:
...
class resizeNormalize():
    ...
    def __call__(self, img):
        ...
        if w <= (w0 / h0 * h):
            ...
            img = self.toTensor(img)
            img.sub_(0.5).div_(0.5)
            start = random.randint(0, w - w_real - 1)  # nosec
            if self.is_test:
                start = 5
                w += 10
...
class randomSequentialSampler(sampler.Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(n_batch):
            random_start = random.randint(0, len(self) - self.batch_size)  # nosec
